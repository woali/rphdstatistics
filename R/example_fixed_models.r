...
##Example - Modeling Claim Severity with GLM Gamma
library(gridExtra)
library(lattice)
...
##Figure
histogram(~auto1$CLAIM_COUNT|X1, panel = function(...) {
        panel.histogram(...)
...
histogram(~auto1$CLAIM_COUNT|X6, panel = function(...) {
        panel.histogram(...)
		panel.abline(v=mean(CLAIM_COUNT), lwd=3, lty=2)
       },xlab="CAPACITY", par.settings=bwtheme)
...
zip1 <- zeroinfl(numclaims ~ gender + as.factor(agecat) + 
...
facet_wrap(~gender)+labs(x="Age category", y="Predicted number of claims")+theme_bw()
