...
summ <-data.frame(panel, "mu.N"=mu.N, u.N="u.N")

##Figure
par(mfrow=c(2,2))
hist(pi[BEGIN_YEAR=="2010"], xlab="A posteriori premium 2010", main="", cex.axis=1.5, cex.lab=1.5)
abline(v=mean(CLAIM_COUNT), lty=2, lwd=3)
hist(EXPOSURE*mu.N[BEGIN_YEAR=="2010"], xlab="A priori premium 2010", main="", cex.axis=1.5, cex.lab=1.5)
abline(v=mean(CLAIM_COUNT), lty=2, lwd=3)
plot(quantile(pi[BEGIN_YEAR=="2010"], probs=seq(0,1,0.005)), lwd=1, type="b", ylab="Quantiles 2010", cex.axis=1.5, cex.lab=1.5)
lines(quantile(EXPOSURE*mu.N[BEGIN_YEAR=="2010"], probs=seq(0,1,0.005)), lwd=3, type="b", xlab="")
legend("topleft", lwd=c(1,3), c("A posteriori", "A priori"), bty = "n")
boxplot(pi~as.factor(BEGIN_YEAR), xlab="", ylab="A posteriori premium 2007-2010", cex.axis=1.5, cex.lab=1.5)
